...
On supervised installations (such as Home Assistant OS), the logs will no longer be written directly to the configuration directory. The logs are sent to systemd journal, and can be found in the UI. This change reduces the ammount of writes to disk.
...
The integraiton has not been working since the API v1 that was used was taken offline in September 2023.
...
On September 8th 2020 IBM announced the withdrawal of it's support for the IBM Watson IoT Platform and successively discontinued all versions until September 30th 2022.

([@tr4nt0r] - [#153567]) ([watson_iot docs])

{% enddetails %}

{% details "OralB integration states and attributes changed" %}
...
Was previously deprecated and logged, and is now changed to failing.

([@gjohansson-ST] - [#153472])

{% enddetails %}

{% details "Mealie version 1 is no longer supported" %}
...
Directly setting state in the `alarm_control_panel` integration is no longer supported. Use the `alarm_state` property instead.
...
`ConfigSource` was deprecated as it was moved to core_config and has now been removed.

([@gjohansson-ST] - [#154112])

{% enddetails %}

{% details "Renault integration no longer created non-functional entities" %}
...
{% details "Asuswrt `last_time_reachable` attribute was removed" %}
...
{% details "Mobile app: name of zone is used instead of the ID" %}

The `mobile_app` integration, which has support for the iPhone and Android companion apps, has been updated to set the device tracker state to the friendly name of the zone when the companion app is set to send only the zone name instead of the exact coordinates.

This means the `person` entity states of custom zones will now be the zone's name instead of the zone's object ID, and might hence break existing automation which trigger on the person state.

As an example, given a zone `zone.kids_school` named `School`.
_Before_ this change: The state of the corresponding `person` and `device_tracker` entities would be `kids_school` when the "Location Sent" in the companion app was set to send "Zone Name Only" and `School` when the  the "Location Sent" in the companion app was set to "Exact".
_After_  this change: The state of the corresponding `person` and `device_tracker` entities is `School` both when the  the "Location Sent" in the companion app is set to "Exact" and when it is set to "Zone Name Only".

([@Ashus] - [#149453]) ([mobile_app docs])

{% enddetails %}

{% details "ONVIF Make Speed optional for GoToPreset action" %}
...
The `neato` integration has been removed from Home Assistant.
