...
On supervised installations (such as Home Assistant OS), we would save logs twice before, once in a log file in your configuration directory, and once in the OS in the systemd journal. All these extra writes to disk would cause more wear on for example SD cards than needed, so we have now stopped doing this.

The logs are now only saved on the OS level, and can be found in the UI.
